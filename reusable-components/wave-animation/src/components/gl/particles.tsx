...
// Simple damp function (fallback if maath causes issues)
function damp(obj: any, prop: string, target: number, smoothness: number, delta: number) {
  const current = obj[prop];
  const diff = target - current;
  obj[prop] = current + diff * (1 - Math.exp(-smoothness * delta * 60));
}
